# Fig 4D — GO Biological Process enrichment dot plot, HNRNPM-KD DOWN-regulated genes.
...
#   (which rebuilds panel_GO_DOWN_BP.png), using the FinalAnalysis enrichment table
#   HnrnpM_DOWN_GO_BP.tsv directly. Top 20 enriched GO BP terms as a dot plot
#   (x = GeneRatio, y = term, size = Count, colour = p.adjust). Sister of Fig_4C.
#
...
GO_TSV   <- file.path(ENR_DIR, "HnrnpM_DOWN_GO_BP.tsv")
...
g <- ggplot(d, aes(x = gr, y = lab, size = Count, colour = p.adjust)) +
  ...
  labs(title = "HNRNPM-KD DOWN-regulated genes: GO Biological Process",
...
ggsave(file.path(OUT_DIR, "Fig_4D.pdf"), g,
       width = W_CM, height = H_CM, units = "cm", scale = SC,
       device = cairo_pdf)
cat(sprintf("Wrote: %s (%d terms; scale=%.3f; axis/legend rendered %.1f pt -> %d pt final)\n",
            file.path(OUT_DIR, "Fig_4D.pdf"), nrow(d), SC, axis_render_pt, AXIS_PT))

# ============================================================
# FIGURE LEGEND (for Fig_4D.pdf)
# ============================================================
# Gene Ontology Biological Process (GO BP) over-representation analysis of the genes
# DOWN-regulated in HNRNPM knockdown (additive ~ group + PSD gene-level DE). Each dot
# is one enriched GO BP term; the top 20 terms by adjusted P value are shown, ordered
# with the highest GeneRatio at the top. The horizontal axis is the GeneRatio (the
# fraction of the queried DOWN gene set annotated to that term), dot size encodes the
# number of DOWN genes in the term (Count), and dot colour encodes the BH-adjusted

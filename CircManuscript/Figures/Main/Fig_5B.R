# Fig 5B — SH-SY5Y growth curve, gHNRNPM_2 vs gCtrl.
...
#   reproduces ONLY the gHNRNPM_2 growth curve (sheet GrowthCurve_gHNRNPM_2_raw),
#   recomputing the mixed two-way RM ANOVA (Greenhouse-Geisser) and per-day
#   Bonferroni t-tests exactly as the source, and drawing the same curve. Sister of Fig_5A.
#
...
SHEET      <- "GrowthCurve_gHNRNPM_2_raw"
LABEL      <- "gHNRNPM_2"
...
#   (as in the previous Fig_5B.pdf), so all four labels are exact. If new data
...
ggsave(file.path(OUT_DIR, "Fig_5B.pdf"), p,
       width = W_CM, height = H_CM, units = "cm", scale = SC,
       device = cairo_pdf)
cat(sprintf("Wrote: %s (scale=%.3f; axis/legend rendered %.1f pt -> %d pt final)\n",
            file.path(OUT_DIR, "Fig_5B.pdf"), SC, axis_render_pt, AXIS_PT))

# ============================================================
# FIGURE LEGEND (for Fig_5B.pdf)
# ============================================================
# Proliferation of SH-SY5Y (dCas9-KRAB) cells over six days after CRISPRi knockdown
# of HNRNPM with guide gHNRNPM_2, compared to a non-targeting control (gCtrl). Points
...
# grey, gHNRNPM_2 in orange). The horizontal axis is day (D0 seeding to D6) and the
...
# at which gCtrl and gHNRNPM_2 differ by a pooled two-sample t-test after Bonferroni

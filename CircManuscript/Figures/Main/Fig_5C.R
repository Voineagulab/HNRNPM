# Fig 5C — EdU % S-phase bar plot, gHNRNPM_1 vs gCtrl, Day 1.
...
#   reproduces ONLY the gHNRNPM_1 Day 1 panel, recomputing the Student's and Welch's
...
# ---- Read the sheet; gHNRNPM_1 Day 1 is cols A/B, rows 5-7 ----
...
ggsave(file.path(OUT_DIR, "Fig_5C.pdf"), pg,
       width = W_CM, height = H_CM, units = "cm", scale = SC,
       device = cairo_pdf)
cat(sprintf("Wrote: %s (scale=%.3f; axis rendered %.1f pt -> %d pt final)\n",
            file.path(OUT_DIR, "Fig_5C.pdf"), SC, axis_render_pt, AXIS_PT))

# ============================================================
# FIGURE LEGEND (for Fig_5C.pdf)
# ============================================================
...
# Student's two-sample two-sided t-test gives P = 1.70 x 10^-2 (*). The caption reports

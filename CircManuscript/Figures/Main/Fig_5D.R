# Fig 5D — EdU % S-phase bar plot, gHNRNPM_2 vs gCtrl, Day 1.
...
#   reproduces ONLY the gHNRNPM_2 Day 1 panel, recomputing the Student's and Welch's
#   two-sample t-tests exactly as the source and drawing the same annotated bar plot.
#   Sister of Fig_5C.R (which is gHNRNPM_1 Day 1).
#
...
# ---- Read the sheet; gHNRNPM_2 Day 1 is cols A/B, rows 12-14 ----
...
ctrl  <- getvals(12:14, 1)
treat <- getvals(12:14, 2)
...
ggsave(file.path(OUT_DIR, "Fig_5D.pdf"), pg,
       width = W_CM, height = H_CM, units = "cm", scale = SC,
       device = cairo_pdf)
cat(sprintf("Wrote: %s (scale=%.3f; axis rendered %.1f pt -> %d pt final)\n",
            file.path(OUT_DIR, "Fig_5D.pdf"), SC, axis_render_pt, AXIS_PT))

# ============================================================
# FIGURE LEGEND (for Fig_5D.pdf)
# ============================================================
# EdU incorporation (percentage of cells in S-phase) one day after CRISPRi knockdown
# of HNRNPM with guide gHNRNPM_2, compared to a non-targeting control (gCtrl), in
# SH-SY5Y (dCas9-KRAB) cells. Bars show the group mean, error bars the standard error
# of the mean, and open circles the individual replicate wells (gCtrl grey, gHNRNPM_2
...
# Student's two-sample two-sided t-test gives P = 7.53 x 10^-4 (***). The caption reports

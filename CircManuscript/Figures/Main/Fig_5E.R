# Fig 5E — proliferation-only circRNA fold-change ceiling, gHNRNPM_1, Day 1 S-phase.
# Adapted from: /mnt/Scratch/PROJECTS/JW_Katana/circRBP_pilot/IV/JW/CircManuscript/w3_circ_FC_ceiling/doubling_vs_genotype.R
# The source draws a 2-page PDF; page 1 ("Main", Day 1) is panel(gHNRNPM_1,Day 1) | panel(gHNRNPM_2,Day 1).
#   This reproduces ONLY the gHNRNPM_1 Day 1 panel. Rather than re-running the growth-window
#   selection, it reads the scalars the source already wrote to manuscript_inputs.csv
#   (R_edu, R_growth, Td_ctrl, exponential-window label, smallest independent-circRNA FC, n_circ)
#   and rebuilds the panel with the identical model curve, ceilings and annotations.
#
# Strategy (same as the Fig_2/Fig_3/Fig_4/Fig_5 series): keep ALL original plot parameters
#   (line/hline widths, annotation text sizes, colours, title/subtitle, bottom legend),
...
Redu    <- getv("R_edu_Day1", GUIDE)   # EdU-derived ceiling ratio at Day 1
...
pg <- ggplot(df, aes(half, fc, colour = src)) +
  ...
  theme(legend.position = "bottom",
        ...
        axis.text   = element_text(size = 0.5 * axis_render_pt),   # -> 4 pt after shrink (matches Fig_S5C/S5D)
        legend.text = element_text(size = 0.5 * axis_render_pt),   # -> 4 pt after shrink
...
ggsave(file.path(OUT_DIR, "Fig_5E.pdf"), pg,
       width = W_CM, height = H_CM, units = "cm", scale = SC,
       device = cairo_pdf)
cat(sprintf("Wrote: %s (scale=%.3f; axis/legend rendered %.1f pt -> %d pt final)\n",
            file.path(OUT_DIR, "Fig_5E.pdf"), SC, axis_render_pt, AXIS_PT))

# ============================================================
# FIGURE LEGEND (for Fig_5E.pdf)
# ============================================================
# Proliferation-only ceiling on the circRNA fold increase that reduced division could
# produce in gHNRNPM_1 knockdown, using the Day 1 EdU data. Under a steady-state

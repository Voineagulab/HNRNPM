# Fig 5F — proliferation-only circRNA fold-change ceiling, gHNRNPM_2, Day 1 S-phase.
...
#   This reproduces ONLY the gHNRNPM_2 Day 1 panel. Sister of Fig_5E.R. Rather than re-running
#   the growth-window selection, it reads the scalars the source already wrote to
#   manuscript_inputs.csv and rebuilds the panel with the identical model curve, ceilings
#   and annotations.
#
...
ggsave(file.path(OUT_DIR, "Fig_5F.pdf"), pg,
       width = W_CM, height = H_CM, units = "cm", scale = SC,
       device = cairo_pdf)
cat(sprintf("Wrote: %s (scale=%.3f; axis/legend rendered %.1f pt -> %d pt final)\n",
            file.path(OUT_DIR, "Fig_5F.pdf"), SC, axis_render_pt, AXIS_PT))

# ============================================================
# FIGURE LEGEND (for Fig_5F.pdf)
# ============================================================
# Proliferation-only ceiling on the circRNA fold increase that reduced division could
# produce in gHNRNPM_2 knockdown, using the Day 1 EdU data. Under a steady-state

# Fig S4B — circRNA x differential-AS overlap by AS class: independent-36 vs universe.
# Adapted from: /mnt/Scratch/PROJECTS/JW_Katana/circRBP_pilot/IV/JW/CircManuscript/d1_circDE_vs_AS/d1_circDE_vs_AS.R
#   (the section that generates grouped_pct_overlap.pdf). Reads the two saved result
...
#   event, in the 36 independent back-splicing circRNAs (red) versus the whole detected
#   universe (grey). Sister of Fig_S4A.R (which is the 56 BSJ-additive-DE version).
#
...
ORIG_W_IN <- 8                       # the source rendered grouped_pct_overlap.pdf at width = 8 in
...
ggsave(file.path(OUT_DIR, "Fig_S4B.pdf"), p,
       width = W_CM, height = H_CM, units = "cm", scale = SC,
       device = cairo_pdf)
cat(sprintf("Wrote: %s (scale=%.3f; axis/legend rendered %.1f pt -> %d pt final)\n",
            file.path(OUT_DIR, "Fig_S4B.pdf"), SC, axis_render_pt, AXIS_PT))

# ============================================================
# FIGURE LEGEND (for Fig_S4B.pdf)
# ============================================================
# Co-localisation of the HNRNPM-independent back-splicing circRNAs with HNRNPM-regulated
# alternative splicing, broken down by alternative-splicing (AS) event class. For each
# rMATS event class (SE skipped exon, MXE mutually exclusive exons, A3SS/A5SS alternative
# 3'/5' splice site, RI retained intron, and "Any" = the union across all five classes),
# bars show the percentage of circRNAs whose back-splice splice site coincides with the
# splice site of at least one significant HNRNPM-differential AS event at the same host
# gene. Two groups are compared per class: the 36 independent back-splicing circRNAs
# (BSJ significant, FSJ non-significant under the additive ~ group + PSD model; red) and
# the whole detected circRNA universe of 5,820 (grey). An AS event is called significant
# at rMATS FDR < 0.05 and |dPSI| >= 0.10. The count above each bar is k/n, where k is the
# number of circRNAs in that group overlapping such an event and n is the number of
# circRNAs in the group. Above each class the sample odds ratio (OR) and the one-sided
# Fisher's exact test P value (alternative, independent-36 more enriched than universe)
# are annotated, printed in grey where P >= 0.05. The horizontal axis is the AS event
# class and the vertical axis is the percentage of circRNAs sharing splice sites with a
# significant AS event. Even these "independent" back-splicing circRNAs, whose competing
# linear (FSJ) junction does not change, are enriched for co-localising with
# HNRNPM-differential AS events overall and specifically at skipped-exon (SE) and
# mutually-exclusive-exon (MXE) events, indicating that FSJ-non-significance does not
# imply independence from HNRNPM-regulated alternative splicing. P values are per class
# and are not corrected for multiple testing.

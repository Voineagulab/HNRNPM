# Fig S4B (new) — circRNA x AS overlap by class, BOTH scopes: independent-36 vs universe.
# Adapted from: /mnt/Scratch/PROJECTS/JW_Katana/circRBP_pilot/IV/JW/CircManuscript/Figures/Supplementary/Fig_S4B.R
...
#   (the make_merged() section that generates grouped_pct_overlap_ANY_merge.pdf, which this
...
#   all-detected-AS pair in a light tint, for the 36 independent back-splicing circRNAs (red)
#   and the whole detected universe (grey). Sister of Fig_S4A_new.R (the 56 BSJ-additive-DE version).
#
...
#   render at the original size, and let ggsave(scale=) squash the figure to 16 x 8.342 cm.
#   The shrink is driven by WIDTH alone, so it is still uniform and the fonts still land where
#   the house convention wants them. H_CM starts from the source panel's 10 x 3.92 in aspect
#   ratio and is then multiplied by H_MULT.
...
#
# DEPARTURES from the source panel grouped_pct_overlap_ANY_merge.pdf, which this
...
#      the PDF is placed at 16 cm wide on the slide. NOTE the y-axis was left at the
...
#   5. HEIGHT: H_MULT = 1.33 makes the figure a third taller than the source panel, by
#      request. Width, SC and every font size are untouched, and because the panel is the
#      only height-flexible row the whole increase goes to the bars.
#   6. A footer line below the legend states the two group sizes; see the footer block.
#   7. The legend is stretched to run from the y axis to the rightmost bar's right edge
#      instead of sitting centred at its natural width; see the legend block.
...
COMPARE <- "independent36_DE vs universe"              # Fisher comparison row (subset vs universe)
STRATUM <- "independent36_DE"                          # summary stratum for the 36 independent circRNAs
ORDER   <- c("SE","MXE","A3SS","A5SS","RI","any")
LAB     <- function(c) ifelse(c == "any", "Any", c)
SIG <- "#d73027"; UNI <- "#4d4d4d"                     # red = independent-36; grey = universe
...
ORIG_W_IN <- 10                      # the source rendered grouped_pct_overlap_ANY_merge.pdf at 10 in
ORIG_H_IN <- 3.92                    # ... and at 3.92 in high
H_MULT    <- 1.33                    # taller than the source panel by this factor, by request
H_CM      <- W_CM * ORIG_H_IN / ORIG_W_IN * H_MULT   # 6.272 -> 8.342 cm (raw PDF 3.92 -> 5.21 in)
...
sub_n     <- unname(getp(SCOPE_SIG, STRATUM,    "any")["n"])   # 36 independent back-splicing circRNAs
...
GAP     <- 0.11 * ymax                              # equal clearance for every pair; halved
                                                    #   from 0.22 by request
...
subtitle_txt <- paste0(
  "Full-strength bars = AS events significant at FDR<0.05 & |dPSI|>=0.10 (as in grouped_pct_overlap.pdf); pale bars = ALL AS events detected by rMATS (as in grouped_pct_overlap_ANY.pdf).\n",
...
# NOTE: the panel is the only height-flexible row, so the panel gives up FOOT_ROW of height
# (FOOT_PT * 1.9 / SC, about 0.54 cm at the final 16 x 8.342 cm) to make room for the footer.
FOOT_L   <- sprintf("HNRNPM-dependent LOI circRNAs: N = %s", format(sub_n, big.mark = ","))
...
ggsave(file.path(OUT_DIR, "Fig_S4B_new.pdf"), g,
...
cat(sprintf("Wrote: %s (scale=%.4f; raw PDF %.2f x %.2f in; legend/x-axis rendered %.1f pt -> %d pt at %g x %g cm)\n",
            file.path(OUT_DIR, "Fig_S4B_new.pdf"), SC, W_CM * SC / 2.54, H_CM * SC / 2.54,
            axis_render_pt, AXIS_PT, W_CM, H_CM))
cat(sprintf("Footer below the legend: \"%s\"  |  \"%s\"\n", FOOT_L, FOOT_R))

# ============================================================
# FIGURE LEGEND (for Fig_S4B_new.pdf)
# ============================================================
# Co-localisation of the linear-output-independent back-splicing circRNAs with alternative
# splicing, broken down by alternative-splicing (AS) event class and shown against two
# definitions of the AS event set. For each rMATS event class (SE skipped exon, MXE mutually
# exclusive exons, A3SS/A5SS alternative 3'/5' splice site, RI retained intron, and "Any" = the
# union across all five classes), bars show the percentage of circRNAs whose back-splice splice
# site coincides with the splice site of at least one AS event at the same host gene. Four bars
# are shown per class. The two full-strength bars count only HNRNPM-regulated AS events, called
# significant at rMATS FDR < 0.05 and |dPSI| >= 0.10; the two pale bars count every AS event
# rMATS detected, with no significance filter. Within each pair, red is the group of 36
# independent back-splicing circRNAs (BSJ significant, FSJ non-significant under the additive
# ~ group + PSD model) and grey is the whole detected circRNA universe of 5,820; both group
# sizes are stated in the legend. The number above each bar is the count of circRNAs in that
# group overlapping at least one such event. Above each pair, the sample odds ratio (OR) and the
# one-sided Fisher's exact test P value (alternative, independent-36 more enriched than
# universe) are annotated in bold, printed in grey where P >= 0.05, and each label is placed the
# same fixed distance above the taller bar of its own pair. The horizontal axis is the AS event
# class and the vertical axis is the percentage of circRNAs overlapping an AS event. Even these
# "independent" back-splicing circRNAs, whose competing linear (FSJ) junction does not change,
# are enriched for co-localising with HNRNPM-differential AS events overall and specifically at
# skipped-exon (SE) and mutually-exclusive-exon (MXE) events, indicating that FSJ-non-significance
# does not imply independence from HNRNPM-regulated alternative splicing. The unfiltered
# comparison is included as a control and not as evidence: because 95.6% of the universe already
# sits at the splice site of some rMATS-tested event, that comparison is saturated, and all 36
# circRNAs match at SE and at Any, which drives the odds ratio to Inf purely through a zero
# denominator rather than through an infinitely strong effect (the accompanying P values, 0.153
# and 0.194, are non-significant). P values are per class and per scope and are not corrected

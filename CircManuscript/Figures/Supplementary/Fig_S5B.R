# Fig S5B — EdU % S-phase bar plot, gHNRNPM_2 vs gCtrl, Day 4.
...
#   reproduces ONLY the gHNRNPM_2 Day 4 panel, recomputing the Student's and Welch's
#   two-sample t-tests exactly as the source and drawing the same annotated bar plot.
#   Sister of Fig_5D.R (gHNRNPM_2 Day 1); Day 4 uses cols D/E.
#
...
# ---- Read the sheet; gHNRNPM_2 Day 4 is cols D/E (4/5), rows 12-14 ----
...
ctrl  <- getvals(12:14, 4)
treat <- getvals(12:14, 5)
...
ggsave(file.path(OUT_DIR, "Fig_S5B.pdf"), pg,
       width = W_CM, height = H_CM, units = "cm", scale = SC,
       device = cairo_pdf)
cat(sprintf("Wrote: %s (scale=%.3f; axis rendered %.1f pt -> %d pt final)\n",
            file.path(OUT_DIR, "Fig_S5B.pdf"), SC, axis_render_pt, AXIS_PT))

# ============================================================
# FIGURE LEGEND (for Fig_S5B.pdf)
# ============================================================
...
# Student's two-sample two-sided t-test gives P = 4.04 x 10^-3 (**). The caption reports

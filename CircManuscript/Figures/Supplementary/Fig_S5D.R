# Fig S5D — proliferation-only circRNA fold-change ceiling, gHNRNPM_2, Day 4 S-phase.
...
#   panel(gHNRNPM_1,Day 4) | panel(gHNRNPM_2,Day 4). This reproduces ONLY the gHNRNPM_2 Day 4
#   panel. Sister of Fig_S5C.R; Day-4 counterpart of Fig_5F.R. Rather than re-running the
#   growth-window selection, it reads the scalars the source already wrote to
#   manuscript_inputs.csv and rebuilds the panel with the identical model curve, ceilings
#   and annotations. (The growth-curve ceiling is timepoint-independent, so only the EdU
#   ceiling ratio differs from Day 1 -> R_edu_Day4.)
#
...
ggsave(file.path(OUT_DIR, "Fig_S5D.pdf"), pg,
       width = W_CM, height = H_CM, units = "cm", scale = SC,
       device = cairo_pdf)
cat(sprintf("Wrote: %s (scale=%.3f; axis/legend rendered %.1f pt -> %d pt final)\n",
            file.path(OUT_DIR, "Fig_S5D.pdf"), SC, axis_render_pt, AXIS_PT))

# ============================================================
# FIGURE LEGEND (for Fig_S5D.pdf)
# ============================================================
# Proliferation-only ceiling on the circRNA fold increase that reduced division could
# produce in gHNRNPM_2 knockdown, using the Day 4 EdU data. Under a steady-state

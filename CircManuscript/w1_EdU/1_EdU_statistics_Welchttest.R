...
# Statistical test
# ----------------
# Two-sample, two-sided t-test comparing gCtrl vs gHNRNPM for each guide/day.
# Welch's t-test (does NOT assume equal variances) is used by default because
# the control and knock-down groups have very different spreads. Set
# VAR_EQUAL <- TRUE below to use the classic (Student's) equal-variance t-test.
#
# Usage
# -----
#   Rscript EdU_statistics.R
...
VAR_EQUAL   <- FALSE                      # FALSE = Welch's t-test (default)
ALTERNATIVE <- "two.sided"                # two-sided: test for any change
OUTPUT_PDF  <- ifelse (VAR_EQUAL, "EdU_statistics_studentttest.pdf",
                       "EdU_statistics_Welchttest.pdf")# output figure
OUTPUT_CSV  <- ifelse (VAR_EQUAL, "stats_studentttest.csv",
                       "stats_Welchttest.csv")         # all t-test results
...
for (i in seq_along(comparisons)) {
  ...
  tt <- t.test(ctrl, treat, alternative = ALTERNATIVE, var.equal = VAR_EQUAL)
  p  <- tt$p.value

  test_name <- if (VAR_EQUAL) "Student's two-sample t-test"
               else           "Welch's two-sample t-test"
  ...
  pg <- ggplot(summ_df, aes(x = x, y = mean, fill = group)) +
    ...
    annotate("text", x = mean(xpos), y = text_y,
             label = paste0("'", stars(p), "   '*", p_expr(p)), parse = TRUE,
...
cat("\n=== EdU % S-phase: gHNRNPM vs gCtrl comparisons ===\n\n")
print(results, row.names = FALSE)
cat("\n")

## --------------------- write all test results to CSV ---------------------- ##
# Every statistical test in this script (the four gCtrl vs gHNRNPM comparisons,
# with group means, SDs, n, the t-statistic, df, p-value and significance) is
# collected in 'results'; save that table.
